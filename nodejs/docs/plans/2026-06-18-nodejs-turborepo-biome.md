...
- Run: `bunx turbo test --filter=@shoutrrr/shoutrrr --force`
...
- `lint/suspicious/noExplicitAny` — in migrated tests using `globalThis.fetch = (async (input: any, init?: RequestInit) => …) as typeof fetch`. Fix: type the param precisely, e.g. `(input: Request | string | URL, init?: RequestInit)`, or import and use `FetchLike` from `@shoutrrr/core`.
