...
describe('the ntfy config', () => {
  ...
  test('non-default config keys (incl. capitalized defaults) are serialized like Go', () => {
    ...
    expect(query).toBe(
      '?cache=Yes&firebase=Yes&markdown=No&priority=Default',
    );
  });
  ...
  test('query values are escaped with Go QueryEscape semantics', () => {
    ...
    const config = new Config();
    config.setURL(new URL(`ntfy://hostname/topic?title=${encodeURIComponent('a*b(c)~d')}`));
  ...
  test.each([
    ...
    ['priority=urgent alias', 'ntfy://host/topic?priority=urgent', Priority.Max],
