...
describe('matrix config', () => {
  test('accepts a URL with the title prop', () => {
    ...
    expect(() =>
      config.setURL(
        new URL('matrix://user:pass@mockserver?rooms=room1&title=Better%20Off%20Alone'),
      ),
    ).not.toThrow();
  });
  ...
  test('returns an error for invalid props', () => {
    const config = new Config();
    expect(() =>
      config.setURL(new URL('matrix://user:pass@mockserver?channels=room1,room2')),
    ).toThrow();
  });
...
function startMockMatrix(): MockServer {
  ...
  const server = Bun.serve({
    ...
    async fetch(req): Promise<Response> {
      ...
      // joins
      if (path === '/_matrix/client/r0/join/%23room1' && req.method === 'POST') {
        return json(200, { room_id: '1' });
      }
      if (path === '/_matrix/client/r0/join/%23room2' && req.method === 'POST') {
        return json(200, { room_id: '2' });
      }
      if (path === '/_matrix/client/r0/join/%23secret' && req.method === 'POST') {
        return json(403, {
          errcode: 'M_FORBIDDEN',
      ...
      // sends (any room id)
      if (/^\/_matrix\/client\/r0\/rooms\/.+\/send\/m\.room\.message\/.+$/.test(path)) {
...
describe('matrix client (real local HTTP server)', () => {
  test('sends a message to joined rooms without errors', async () => {
    ...
    service.initialize(new URL(`matrix://user:pass@${active.host}?disableTLS=Yes`));
    await service.send('Test message', undefined);
    const sends = active.captured.filter((c) => c.method === 'PUT');
    expect(sends.length).toBe(1);
    expect(JSON.parse(sends[0]!.body)).toEqual({ msgtype: 'm.text', body: 'Test message' });
  ...
  test('sends to explicit rooms, joining aliases first', async () => {
    active = startMockMatrix();
    const service = new MatrixService();
    service.initialize(new URL(`matrix://user:pass@${active.host}?disableTLS=Yes&rooms=room1,room2`));
    await service.send('Test message', undefined);
    const joins = active.captured.filter((c) => c.method === 'POST' && c.rawPath.includes('/join/'));
    const sends = active.captured.filter((c) => c.method === 'PUT');
  ...
  test('reports an error when one room cannot be joined', async () => {
    active = startMockMatrix();
    const service = new MatrixService();
    service.initialize(new URL(`matrix://user:pass@${active.host}?disableTLS=Yes&rooms=secret,room2`));
  ...
  test('login body uses the default device ID and password identifier', async () => {
    ...
    const loginPost = active.captured.find(
      (c) => c.method === 'POST' && c.rawPath.startsWith('/_matrix/client/r0/login'),
    );
  ...
  test('does not crash without a logger', async () => {
    ...
    const service = new MatrixService();
    service.initialize(new URL(`matrix://user:pass@${active.host}?disableTLS=Yes`));
    await expect(service.send('Test message', undefined)).resolves.toBeUndefined();
  });
  ...
  test('uses an access token directly when no user is provided', async () => {
    ...
    service.initialize(new URL(`matrix://:TOKEN@${active.host}?disableTLS=Yes`));
    await service.send('Test message', undefined);
    const sends = active.captured.filter((c) => c.method === 'PUT');
    expect(sends.length).toBe(1);
    // no login should have occurred (token used directly)
    const loginGet = active.captured.find((c) => c.rawPath.startsWith('/_matrix/client/r0/login'));
...
describe('matrix regressions', () => {
  test('room IDs containing `$` are not corrupted by String.replace (finding 1)', async () => {
    ...
    try {
      const service = new MatrixService();
      service.initialize(new URL(`matrix://:TOKEN@localhost:${server.port}?disableTLS=Yes`));
  ...
  test('repeated query key uses the first value (Go vals[0] semantics, finding sweep)', () => {
    const config = new Config();
    config.setURL(new URL('matrix://user:pass@mockserver?rooms=room1&rooms=room2'));
  ...
  test('login retries after a transient failure (finding 2)', async () => {
    ...
    const server = Bun.serve({
      ...
      async fetch(req): Promise<Response> {
        ...
        if (path === '/_matrix/client/r0/login' && req.method === 'POST') {
          return json(200, { access_token: 'TOKEN', home_server: 'h', user_id: 'u' });
        }
        if (path === '/_matrix/client/r0/joined_rooms') {
    ...
    try {
      const service = new MatrixService();
      service.initialize(new URL(`matrix://user:pass@localhost:${server.port}?disableTLS=Yes`));
      ...
      await expect(service.send('hi again', undefined)).resolves.toBeUndefined();
    } finally {
      server.stop(true);
    }
  });
