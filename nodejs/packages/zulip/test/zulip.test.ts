...
describe('the zulip config', () => {
  describe('given a service url with missing parts', () => {
    it('should error if bot mail is missing', () => {
      ...
      expect(() => createConfigFromURL(url)).toThrow(ErrorMessage.MissingBotMail);
    });

    it('should error if api key is missing', () => {
      const url = new URL('zulip://bot-name%40zulipchat.com@example.zulipchat.com?stream=foo&topic=bar');
      expect(() => createConfigFromURL(url)).toThrow(ErrorMessage.MissingAPIKey);
    });
    ...
    it('should error if host is missing', () => {
      ...
      expect(() =>
        createConfigFromURL(new URL('zulip://bot-name%40zulipchat.com:correcthorsebatterystable@?stream=foo')),
      ).toThrow();
    });
  });
  ...
  describe('parsing a valid service url', () => {
    it('should map all fields', () => {
      const config = createConfigFromURL(
        new URL('zulip://bot-name%40zulipchat.com:correcthorsebatterystable@example.zulipchat.com?stream=foo&topic=bar'),
      );
  ...
  describe('building a service url', () => {
    ...
    it('should preserve a non-standard port through a round-trip (fix #495)', () => {
      const config = createConfigFromURL(
        new URL('zulip://bot-name%40zulipchat.com:correcthorsebatterystable@example.zulipchat.com:8443?stream=foo&topic=bar'),
      );
...
describe('sending messages', () => {
  ...
  beforeEach(() => {
    ...
    server = Bun.serve({
      ...
      async fetch(req) {
        ...
        return new Response(nextStatus === 200 ? '' : 'bad payload', { status: nextStatus });
      },
    });
  ...
  it('should POST form values to /api/v1/messages with basic auth', async () => {
    ...
    const expectedAuth =
      'Basic ' + Buffer.from('bot-name@zulipchat.com:correcthorsebatterystable').toString('base64');
  ...
  it('should report the transport error when the API cannot be reached', async () => {
    ...
    const service = new ZulipService({ apiOrigin: origin });
    service.initialize(new URL(BASE_URL));
    await expect(service.send('This is a message')).rejects.toThrow(/failed to send zulip message/);
  });
});
