...
import { GotifyService, buildURL, isTokenValid } from '../src/gotify.js';
...
describe('creating a config', () => {
  it('should be identical after de-/serialization (with path)', () => {
    const testURL = 'gotify://my.gotify.tld/gotify/Aaa.bbb.ccc.ddd?title=Test+title';
    const config = new Config();
  ...
  it('should reject a non-integer priority (Go strconv.ParseInt parity)', () => {
    ...
    expect(() =>
      config.setURL(new URL('gotify://my.gotify.tld/Aaa.bbb.ccc.ddd?priority=5x')),
    ).toThrow();
  });
});
 ...
 */
function startMockGotify(status: number, responseBody: unknown): {
  server: MockServer;
  host: string;
...
describe('sending the payload', () => {
  ...
  it('should reject when a 2xx response carries a non-JSON body (Go parseResponse parity)', async () => {
    ...
    const service = new GotifyService();
    service.initialize(
      new URL(`gotify://localhost:${server.port}/Aaa.bbb.ccc.ddd?disabletls=Yes`),
    );
