...
import { ErrorInvalidToken, ErrorMismatchedTokenSeparators } from '../src/errors.js';
...
describe('Token', () => {
  describe('parse / normalize', () => {
    ...
    it('normalizes an xoxb API token', () => {
      const token = parseToken('xoxb:AAAAAAAAA-BBBBBBBBB-123456789123456789123456');
    ...
    it('throws ErrorInvalidToken when part A is not at least 9 chars', () => {
      expect(() => parseToken('12345678/123456789/123456789123456789123456')).toThrow(
        ErrorInvalidToken,
      );
    });

    it('throws ErrorInvalidToken when part C is not at least 24 chars', () => {
      expect(() => parseToken('123456789/123456789/12345678912345678912345')).toThrow(
        ErrorInvalidToken,
      );
    });
