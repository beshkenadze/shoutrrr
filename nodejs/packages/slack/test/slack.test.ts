...
describe('creating the payload', () => {
  ...
  describe('attachment chunking', () => {
    it('appends the exceeding lines to the last attachment when over 99 lines', () => {
      ...
      expect((atts[atts.length - 1] as { text: string }).text).toContain('Line 110');
    });
...
describe('sending the payload', () => {
  describe('via webhook URL', () => {
    const webhookURL = 'slack://testbot@AAAAAAAAA/BBBBBBBBB/123456789123456789123456';
    ...
    it('rejects if the server returns a non-ok body', async () => {
      ...
      await expect(service.send('Message')).rejects.toThrow('webhook response: invalid_payload');
    });
  ...
  describe('via bot API', () => {
    const apiURL = 'slack://xoxb:123456789012-1234567890123-4mt0t4l1YL3g1T5L4cK70k3N@C0123456789';
    ...
    it('rejects when the API responds ok:false', async () => {
      ...
      const service = new SlackService();
      service.setFetch(
        mockFetch(200, JSON.stringify({ ok: false, error: 'someone turned off the internet' }), recorder),
      );
      service.initialize(new URL(apiURL));
