...
function recordingTransport(): { transport: TransportLike; sent: MailMessage[] } {
...
describe('config validation', () => {
  it('rejects a URL missing fromAddress', () => {
    ...
    expect(() =>
      config.setURL(
        new URL('smtp://user:password@example.com:2225/?toAddresses=rec1@example.com,rec2@example.com'),
      ),
    ).toThrow(/fromAddress/);
  });
  ...
  it('rejects a URL missing toAddresses', () => {
    const config = new Config();
    expect(() =>
      config.setURL(new URL('smtp://user:password@example.com:2225/?fromAddress=sender@example.com')),
    ).toThrow(/toAddress/);
  });
  ...
  it('rejects a non-numeric port-like value in a uint query field', () => {
    ...
    expect(() =>
      config.setURL(
        new URL('smtp://host:25/?usestarttls=maybe&fromaddress=s@example.com&toaddresses=r@example.com'),
      ),
  ...
  it('rejects an invalid enum value', () => {
    ...
    expect(() =>
      config.setURL(
        new URL('smtp://example.com/?fromAddress=s@example.com&toAddresses=r@example.com&auth=bogus'),
      ),
    ).toThrow(/None, Plain/);
  });
});
...
describe('AuthType enum', () => {
  it('parses case-insensitively and prints by index', () => {
    ...
    expect(authTypeFormatter.names()).toEqual(['None', 'Plain', 'CRAMMD5', 'Unknown', 'OAuth2']);
  });
});

describe('Encryption enum', () => {
  it('parses case-insensitively and prints by index', () => {
    expect(encryptionFormatter.parse('explicittls')).toBe(Encryption.ExplicitTLS);
    expect(encryptionFormatter.parse('Auto')).toBe(Encryption.Auto);
    expect(encryptionFormatter.parse('xxx')).toBe(-1);
    expect(encryptionFormatter.print(Encryption.ImplicitTLS)).toBe('ImplicitTLS');
  });
...
describe('initialize auth resolution', () => {
  it('resolves Unknown to Plain when a username is present', () => {
    ...
    service.initialize(
      new URL('smtp://user:pass@host:587/?fromaddress=s@example.com&toaddresses=r@example.com'),
    );
...
describe('transport options derivation', () => {
  it('uses implicit TLS for port 465 with Auto encryption', () => {
    const config = new Config();
    config.setURL(
      new URL('smtp://user:pass@host:465/?fromaddress=s@example.com&toaddresses=r@example.com'),
    );
  ...
  it('uses opportunistic StartTLS when enabled on a non-implicit session', () => {
    const config = new Config();
    config.setURL(
      new URL('smtp://host:587/?usestarttls=yes&auth=none&fromaddress=s@example.com&toaddresses=r@example.com'),
    );
    const opts = buildTransportOptions(config);
  ...
  it('disables any upgrade when StartTLS is off on a plain session', () => {
    const config = new Config();
    config.setURL(
      new URL('smtp://host:587/?usestarttls=no&encryption=ExplicitTLS&auth=none&fromaddress=s@example.com&toaddresses=r@example.com'),
    );
    const opts = buildTransportOptions(config);
    expect(opts.secure).toBe(false);
  ...
  it('sets CRAM-MD5 auth method', () => {
    const config = new Config();
    config.setURL(
      new URL('smtp://u:p@host:25/?auth=CRAMMD5&fromaddress=s@example.com&toaddresses=r@example.com'),
    );
    const opts = buildTransportOptions(config);
...
describe('message envelope', () => {
  ...
  it('adds an HTML alternative when useHTML is set', () => {
    const config = new Config();
    config.setURL(
      new URL('smtp://host:25/?fromaddress=s@example.com&toaddresses=r@example.com&usehtml=yes'),
    );
...
describe('send via injected transport', () => {
  ...
  it('applies send-time params', async () => {
    ...
    service.initialize(
      new URL('smtp://host:25/?fromaddress=s@example.com&toaddresses=r@example.com&subject=Default'),
    );
  ...
  it('throws when an invalid send param is passed', async () => {
    ...
    const service = new SmtpService(() => rec.transport);
    service.initialize(
      new URL('smtp://host:25/?fromaddress=s@example.com&toaddresses=r@example.com'),
    );
    await expect(service.send('msg', { invalid: 'value' })).rejects.toThrow(/applying params/);
  });
